alias lsa='ls -lah'
alias gbd='go build -ldflags="-s -w"'
alias zk='zk -W ~/cyb/note/'
alias pandoc='pandoc --template ~/.local/share/pandoc/template.html -s -f markdown -t html -V mainfont="Chakra Petch" -V fontsize=25px --metadata-file ~/.local/share/pandoc/metadata.yaml --toc'
alias note='cd ~/cyb/notes/'
alias pass='passage'
alias rami='shred -fzuv $@'
alias li='ls -li'
alias r='ranger $@'
# alias m='mpv'
alias feh='feh --conversion-timeout 1'
alias gimp='((gimp > /dev/null 2>&1)&)'
alias jtl='journalctl -f'
alias ggc='cd ~/git/playground/'
alias px='pwd | xclip' 
alias ..='cd .. 2>/dev/null || cd "$(dirname $PWD)"'
alias mouse='$zero modprobe -r psmouse && $zero modprobe psmouse'
...
alias pt='$zero prt-get'
alias pgf='prt-get fsearch'
alias pgi='$zero prt-get install'
...
alias lh='ls -lh'
alias pa='$zero pkgadd'
alias ytvid='yt-dlp --restrict-filenames -o "~/mus/vid/%(title)s_%(width)sx%(height)s_%(upload_date)s.%(ext)s"'
alias ytaudio='yt-dlp --cookies-from-browser chrome --restrict-filenames -x --audio-quality 0 --audio-format mp3 -o "~/mus/%(title)s_%(width)sx%(height)s_%(upload_date)s.%(ext)s"'
alias sit='cd ~/git/mysite/'
alias api='$zero apt install --yes -V'
alias aps='apt search $1'
alias spy='lsof -i -P +c 0 +M'
alias netlist='lsof -i -P | grep LISTEN'
alias apg='$zero apt purge -V'
...
#alias mk='$EDITOR /etc/portage/make.conf'
#alias py='pyscript'
...
#alias nv='$EDITOR /etc/nixos/configuration.nix'
...
#alias git='hub'
alias gs='git status'
alias apar='$zero apt autoremove -V'
...
alias cdd='cd ~/.cfg'
alias q='exit'
alias fk='$EDITOR ~/.zsh/functions.zsh'
alias plz='$zero $(fc -ln -1)'
...
alias vp='$EDITOR $HOME/.config/polybar/config.ini'
alias cp='cp -i'
alias mv='mv -i'
...
# Install the package only if its not updated/already installed
alias si='$zero pacman -S --needed'
# Remove the package
alias sr='$zero pacman -R'
# Remove the package with dependencies not required by other packages too.
alias srs='$zero pacman -Rs'
# Remove the package, dependencies and also those who depend on it
alias srsc='$zero pacman -Rsc'
# Remove the package with no .pacsave configurations being left.
alias srn='$zero pacman -Rn'
# Update the system
alias update='$zero pacman -Syu'
...
# Clean cache
alias sC='$zero pacman -Scc'
# Download a package without installing it 
alias sdw='$zero pacman -Sw'
# Install local package
alias sil='$zero pacman -U'
alias awq='$EDITOR $HOME/.config/qtile/config.py'
alias vkt='$EDITOR $HOME/.config/kitty/kitty.conf'
alias awb='$EDITOR $HOME/.config/bspwm/bspwmrc'
alias awh='$EDITOR $HOME/.config/herbstluftwm/autostart'
...
alias cdpo='cd $HOME/.config/polybar'
alias ll='ls -l'
alias lss='ls -lh'
alias k='clear'

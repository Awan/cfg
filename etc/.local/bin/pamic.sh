...
# finds the active source for pulse audio and increments the volume. useful when you have multiple audio outputs and have a key bound to vol-up and down
...
function volUp {
    ...
    then
        pactl set-source-volume "$active_source" "$maxvol%"
...
function getSinkInputs {
    input_array=$(pacmd list-source-inputs | grep -B 4 "source: $1 " | awk '/index:/{print $2}')
}

function volSync {
    getSinkInputs "$active_source"
    getCurVol

    for each in $input_array
    do
        pactl set-source-input-volume "$each" "$curVol%"
    done
}
